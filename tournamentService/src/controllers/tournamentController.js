...
router.post("/tournament/team", async (req, res) => {
...
router.put("/tournament/team/position", async (req, res) => {
...
router.post("/game", async (req, res) => {
    ...
    try {
        const result = await tournamentModel.addGame({ game_id, game_name });
        res.status(201).json(createResponse("success", result, "Game added successfully"));
    } catch (error) {
        console.error("Error in Add Game API:", error.message);
        res.status(500).json(createResponse("error", null, error.message));
    }
});
